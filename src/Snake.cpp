...
void Snake::setIsMultiplayerGame(bool isMultiplayer)
{
...
void Snake::logic(int fruitX, int fruitY, int secFruitX, int secFruitY, std::vector<Fruit*> fruitVec, int coordMultiX, int coordMultiY, int* multiTailX, int* multiTailY, int multiTailSize )
{
    fruitFound=false;
    int prevX = tailX[0];
    int prevY = tailY[0];
    ...
    case PAUSE:
       pausedGame=true;
       std::cout<< "PAUSE  PRESS ANY LETTER TO CONTINUE"<<std::endl;
       this->input();
       break;
    default:
        break;
    ...
    for (int i = 0; i < tailSize; i++)
        if (tailX[i] == coordinateX && tailY[i] == coordinateY && pausedGame==false)
            gameOver = true;


    if (coordMultiX!=-1)
    {
        if (coordMultiX==coordinateX && coordMultiY==coordinateY)
        {
            gameOver=true;
        }
    }
    ...
    if (multiTailSize!=-1)
    {
        for (int i = 0; i < multiTailSize; i++){
            if (multiTailX[i] == coordinateX && multiTailY[i] == coordinateY)
            {
